class ExportationController {
    ...
    async createExportation(req, res) {
      try {
        const exportation = await this.exportationService.createExportation(req.body);
    ...
    async getOneExportation(req, res) {
      try {
        const Exportation = await this.exportationService.getExportationById(req.params.id);
        if (Exportation) {
