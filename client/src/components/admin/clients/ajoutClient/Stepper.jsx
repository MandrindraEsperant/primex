...
const Stepper = ({ steps, currentStep }) => {
    ...
    const displayStep = newStep.map((step, index) => {
        return (
            <div key={index} className={ index != newStep.length - 1 ? 'w-full flex items-center' :'flex items-center'}>
                ...
                <div className={`flex-auto border-t-2 transition duration-500 easi-in-out ${step.completed ? "border-green-600" : "border-gray-300"
                    }`}>
                    {/* Display line */}
    ...
    return (
        <div style={{width:'100%'}} className='mx-4 p-6 flex justify-between items-center displayStep'>
            {displayStep}
        </div>
    )
}
